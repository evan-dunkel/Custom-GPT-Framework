<script lang="ts">
  ...
  const handleFormSubmit = async (e: SubmitEvent) => {
	...
	<div class="flex flex-col flex-1 gap-4 overflow-auto p-4">
        <slot />
        {#each $messages as message}
            <div class="w-full">
                <Card.Root class="{message.role === 'assistant' ? 'justify-self-start' : 'justify-self-end'} w-3/5">
                    <Card.Content>
    ...
    <div class="flex flex-col gap-2 px-4 py-4 border-t border-border">
        <form class="flex items-center gap-2" on:submit={handleFormSubmit}>
